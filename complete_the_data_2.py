...
def complete_the_data(posts, users):
    output = []
    modified_users = {}
    for user in users:
        modified_users[user["user_id"]] = user["name"]
    for post in posts:
        output.append(
            {"title": post["title"], "submitted_by": modified_users[post["submitted_by"]], "likes": post["likes"]})
    return output


print(complete_the_data([
    {"title": 'Me Eating Pizza', "submitted_by": 231, "likes": 1549},
